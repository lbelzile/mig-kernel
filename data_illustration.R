...
bw1 <- mig_kdens_bandwidth(
   x = post,
   beta = beta,
   type = "full",
   method = "lcv"
)
bw2 <- mig_kdens_bandwidth(
   x = post,
   beta = beta,
   type = "full",
   method = "amise",
   approx = "mig",
   N = 1e4L)
# Check correlation between entries
cov2cor(bw1)
cov2cor(bw2)
bw3 <- mig_kdens_bandwidth(
   x = post,
   beta = beta,
...
g2 <- ggplot() +
   ...
   geom_contour(
      data = z,
      mapping = aes(x = x, y = y, z = mixtdens2),
      breaks = max(z$mixtdens2) - qchisq(seq(0.05, 0.95, by = 0.10), df = 2),
      col = "grey50"
   ) +
   # geom_contour(
   #    data = z,
